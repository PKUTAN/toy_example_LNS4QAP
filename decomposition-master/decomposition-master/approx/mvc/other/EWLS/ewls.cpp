...
void cover_LS()
{
	...
	while (step<max_steps)
	{
		...
		if (step - last_find_min_step == 1000000)
		{
			if(ub - c_size <delta_size)
				update_target_size();
		}
	}
}
...
int main(int argc, char* argv[])
{
	...
	cout<<"Program terminated in "<<comp_time<<" seconds"<<endl;
	
	//check solution
	if(check_solution()==1)
		cout<<"the output solution is a MIS."<<endl;
	else
		cout<<"the output solution is not a MIS!!"<<endl;
